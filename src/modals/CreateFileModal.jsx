    ...
    const handleOk = () => {
        form.validateForm().then((res) => {
            ...
            const submittedParentId = (parentFolderId === "null" || !parentFolderId)
                ...
                : parentFolderId;
            folder.addFile.mutateAsync({ ...form.values, parentId: submittedParentId || null }).then(() => {
                disAppear(modalId)
            })
    ...
    return (
        <div>
            <form onSubmit={form.handleSubmit}>
                ...
                <div className='flex flex-col'>
                    <input id='url' name='url' onChange={form.handleChange} value={form.values.url} className='bg-white' placeholder='Url giriniz' />
                    <span id='input-error'>{form.errors.url}</span>
                </div>
