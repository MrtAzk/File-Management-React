import axios from "axios";

export const instance = axios.create({
    baseURL: "http://localhost:9090"
})

export const listFile = async (query) => {

    try {
        const res = await instance.get("/file", { params: query });
        return res.data;
    } catch (error) {
        console.log("Dosyalar getirilirken hata oluştu ");
        throw error;
    }

}

export const createFile = async (data) => {

    try {
        const res = await instance.post("/file", data);
        return res.data
    } catch (error) {
        console.log("Dosyalar yaratılırken  hata oluştu ");
        throw error;
    }
}

export const findByIdFile = async (id) => {

    try {
        const res = await instance.get(`/file/${id}`);
        return res.data
    } catch (error) {
        console.log("Aranan Dosya  hata oluştuşturdu ");
        throw error;
    }

    
}

export const updateFile = async (id,data) => {

    try {
        const res = await instance.put(`/folder/${id}`,data);
        return res.data
    } catch (error) {
        console.log("Dosyalar güncelemesinde  hata oluştu ");
        throw error;
    }
}

export const patchFile = async (id,data) => {

    try {
        const res = await instance.patch(`/folder/${id}`,data);
        return res.data
    } catch (error) {
        console.log("Dosyalar güncellenirken  hata oluştu ");
        throw error;
    }
}

export const removeFile = async (id) => {

    try {
        const res = await instance.delete(`/folder/${id}`);
        return res.data
    } catch (error) {
        console.log("Dosyalar silinirken  hata oluştu ");
        throw error;
    }
}